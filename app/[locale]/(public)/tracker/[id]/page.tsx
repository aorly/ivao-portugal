...
export default async function TrackerSessionPage({ params }: Props) {
  ...
  const station = pickString(session?.station, session?.position, session?.callsign)?.toUpperCase() ?? "";
  const connectionType = pickString(session?.connectionType)?.toUpperCase() ?? "";
  ...
  const resolveAtcFrequency = async () => {
    if (connectionType !== "ATC" || !station) return null;
    const normalized = station.replace(/[^A-Z0-9_]/g, "").toUpperCase();
    if (!normalized) return null;
    const exact = await prisma.atcFrequency.findFirst({
      where: { station: normalized },
    ...
    if (!airport || !role) return null;
    return prisma.atcFrequency.findFirst({
      where: {
        AND: [{ station: { startsWith: `${airport}_` } }, { station: { contains: `_${role}` } }],
      },
      orderBy: { station: "asc" },
      include: { boundaries: { include: { points: { orderBy: { order: "asc" } } } } },
    });
  };
  const atcFrequency = await resolveAtcFrequency();
  const atcBoundaries = (atcFrequency?.boundaries ?? [])
    .map((boundary) => ({
      id: boundary.id,
      label: `${station} ${atcFrequency?.frequency ?? ""}`.trim(),
  ...
  const mapTitle = atcBoundaries.length > 0 ? "ATC boundary" : "Flight track";
