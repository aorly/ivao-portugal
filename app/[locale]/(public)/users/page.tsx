...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: "Active division members.",
    alternates: { canonical: absoluteUrl(`/${locale}/users`) },
  };
}
...
export default async function DivisionUsersPage() {
  ...
  const raw = await ivaoClient.getDivisionUsers(divisionId);
  const apiFailed = Array.isArray(raw) && raw.length === 0;
  const users = asArray(raw).map((entry) => entry as Record<string, unknown>);
    ...
    <main className="flex flex-col gap-6">
      <div className="mx-auto w-full max-w-6xl">
        <SectionHeader
          eyebrow="Division"
          title="Division users"
          description={`Members registered in IVAO ${divisionId}.`}
          action={null}
        />
        ...
        <Card className="border border-[color:var(--border)] bg-[color:var(--surface)] p-4">
          {apiFailed ? (
            <div className="mb-4 rounded-xl border border-[color:rgba(233,52,52,0.3)] bg-[color:rgba(233,52,52,0.12)] px-3 py-2 text-xs font-semibold text-[color:#7a1e1e]">
              IVAO API error while loading division users. Please try again later.
            </div>
          ) : null}
          {users.length === 0 ? (
            <p className="text-sm text-[color:var(--text-muted)]">No users available.</p>
          ) : (
            <div className="overflow-x-auto rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface-2)]">
              <div className="min-w-[520px]">
                <div className="grid grid-cols-[0.7fr_1.4fr_1fr_1fr] gap-2 border-b border-[color:var(--border)] px-4 py-3 text-[11px] uppercase tracking-[0.14em] text-[color:var(--text-muted)]">
                  <span>VID</span>
                  <span>Name</span>
                  <span>Rating</span>
                  <span>Status</span>
                ...
                <div className="divide-y divide-[color:var(--border)]">
                  {users.map((entry) => {
                    ...
                    return (
                      <div key={vid} className="grid grid-cols-[0.7fr_1.4fr_1fr_1fr] gap-2 px-4 py-3 text-sm">
